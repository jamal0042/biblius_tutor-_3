...
interface ExampleInfoRetour {
    barcode: string
    documents: MaybeArray<DocInfoRetour>
}
...
interface RetourData {
    ...
    prets: MaybeArray<{
        due_date: string
        loan_date: string
        members: MaybeArray<MemberInfo>
        exemplaires: MaybeArray<ExampleInfoRetour>
    }>
}
...
function getTitle(doc: DocInfoRetour | null | undefined): string {
    return doc?.title || "Inconnu"
}

function getAuthorName(doc: DocInfoRetour | null | undefined): string {
    return toSingle(doc?.auteurs)?.name || ""
}
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {typedPretsEnRetard.map((pret) => {
                                        const ex = toSingle(pret.exemplaires)
                                        const doc = ex ? toSingle(ex.documents) : null
                                        const member = toSingle(pret.members)
                                        ...
                                        return (
                                            <tr key={pret.id} className="hover:bg-red-50/50 dark:hover:bg-red-950/20 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="font-medium text-slate-900 dark:text-white">{getTitle(doc)}</div>
                                                    <div className="text-sm text-slate-500 dark:text-slate-400">{getAuthorName(doc)}</div>
                                                </td>
                                                <td className="px-6 py-4">
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {typedRetours.map((retour) => {
                                        const pret = toSingle(retour.prets)
                                        const ex = pret ? toSingle(pret.exemplaires) : null
                                        const doc = ex ? toSingle(ex.documents) : null
                                        const member = pret ? toSingle(pret.members) : null
