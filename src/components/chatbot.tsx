    ...
    try {
        ...
        return null // En cas d'erreur, on retombe sur les réponses locales
    }
    }

    // Moteur de réponses HYBRIDE : données réelles en local, le reste via Gemini
    const getBotResponse = async (userMessage: string): Promise<string> => {
    const msg = userMessage.toLowerCase().trim()

    // ===== 1. DONNÉES RÉELLES (traitées en local car Gemini n'accède pas à la base) =====
    ...
    // ===== 2. TOUT LE RESTE → GEMINI =====
    const geminiReply = await callGemini(userMessage)
    if (geminiReply) return geminiReply

    // ===== 3. FALLBACK local si Gemini échoue =====
